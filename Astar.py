...
def make_grid(rows,width):
    ...
    for i in range(0,699,700//rows):
        tmp, temp = get_clicked_pos((699,699), rows, width)
        cord, coord = get_clicked_pos((i,i), rows, width)
        spot = grid[0][cord]
        spot.make_barrier()
...
def remove_path(win,grid,rows,width):
    for i in range(rows):
        for j in range(rows):
            if grid[i][j].is_path() or grid[i][j].is_closed() or grid[i][j].is_open():
                grid[i][j].reset()


def main(win,width):
    ROWS = 50 # Free to change
    ...
    while run:
        ...
        for event in pg.event.get():
            ...
            if pg.mouse.get_pressed()[0]: # Left click
                ...
                spot = grid[row][col]
                if not start and spot != end and row != 0 and col != 0 and row != ROWS-1 and col != ROWS-1:
                    start = spot
                    remove_path(win, grid, ROWS, width)
                    start.make_start()
                
                elif not end and spot != start and row != 0 and col != 0 and row != ROWS-1 and col != ROWS-1:
                    remove_path(win, grid, ROWS, width)
                    end = spot
                    end.make_end()
                ...
                row, col = get_clicked_pos(pos, ROWS, width)
                spot = grid[row][col]
                if row != 0 and col != 0 and row != ROWS-1 and col != ROWS-1:
                    if spot.color != WHITE:
                        remove_path(win, grid, ROWS, width)
                    spot.reset()
